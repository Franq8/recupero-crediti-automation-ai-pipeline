...
async function run() {
  ...
  const rowsRes = await fetch(`${API}/practices/${practiceId}/table-rows`);
  const rowsJson = await rowsRes.json();
  if (!rowsRes.ok) throw new Error('table-rows failed');
  if ((rowsJson.data ?? []).length !== 2) throw new Error('table rows count mismatch');
  ...
  if (docBytes.byteLength < 300) throw new Error('Generated row DOCX too small');

  const csvRes = await fetch(`${API}/practices/${practiceId}/table-rows/export.csv`);
  if (!csvRes.ok) throw new Error('table export csv failed');
  const csvOut = await csvRes.text();
  if (!csvOut.includes('row_id')) throw new Error('table export malformed');
